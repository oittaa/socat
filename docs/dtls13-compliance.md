...
| **§4.4** Handshake SHOULD fragment if too big; SHOULD shrink after unanswered retries when PMTU unknown | yes: fragments to the association budget; `EMSGSIZE` and unanswered handshake flights shrink with a bound; opt-in DF RRC probes confirm and search upward | fragments and re-queries on MTU error; DTLS 1.3 loss backoff unknown | fragments; loss backoff unknown | fragments; loss backoff unknown |
...
| --- | --- | --- |
| Dynamic PMTU handling | RFC 9147 §4.4 / RFC 8899 | Handshake shrink plus opt-in confirm/search on dedicated DF sockets. Manual probes still do not raise the working size. No ICMP PTB, no `IP_MTU` query, no DF on shared listeners. |
