...
| **§4.4** Handshake SHOULD fragment if too big; SHOULD shrink after unanswered retries when PMTU unknown | yes: fragments to the association budget; `EMSGSIZE` and unanswered handshake flights shrink with a bound; padded RRC probes exist but do not search upward yet | fragments and re-queries on MTU error; DTLS 1.3 loss backoff unknown | fragments; loss backoff unknown | fragments; loss backoff unknown |
...
| --- | --- | --- |
| Dynamic PMTU handling | RFC 9147 §4.4 / RFC 8899 | Handshake shrink is implemented. Padded RRC probes exist when RRC and dedicated-socket `PMTUDISC_PROBE` are present; matching a probe does not raise the usable size yet. No `IP_MTU` query or DF on shared listeners. |
