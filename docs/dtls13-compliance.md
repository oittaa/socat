...
| **§4.4** MUST report transport PMTU-exceeded errors; SHOULD allow app DF control | application `Write` returns wrapped `EMSGSIZE`; handshake consumes it and fragments. Caller owns socket DF; only `UnfragmentedProbes` on a dedicated client sets DF | MTU-error handling and DF BIO controls; see evidence below | unknown | unknown |
| **§4.4** Handshake SHOULD fragment if too big; SHOULD shrink after unanswered retries when PMTU unknown | yes: fragments to the association budget; `EMSGSIZE` and a second unanswered handshake retry shrink with a bound | fragments and re-queries on MTU error; DTLS 1.3 loss backoff unknown | fragments; loss backoff unknown | fragments; loss backoff unknown |
...
| **§5.5** Timer SHOULD be 3×RTT or 1s | old path: 3×RTT, 100ms floor, else 1s; candidate: at least 1s | n/a | n/a | yes: 1s |
