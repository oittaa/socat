...
**`shut-none` / `shut-down` / `shut-close` / `shut-null`:** classic `GROUP_FD` / `PH_OFFSET` `howtoshut` (`xio-fd.c` / `xioshutdown.c`, tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same tree). One ordered field: last active occurrence wins, including Go-only `shut=none|down|close|null`. Omitted value selects; `=0`/`false`/`no`/`off` does not. `none` makes `ShutdownWrite` a no-op (and EXEC does not kill the child); `down` performs socket-style `shutdown(SHUT_WR)`; `close` fully closes; `null` sends a zero-length datagram. Man documents `shut-none[=<bool>]`; C `TYPE_CONST` rejects any assignment. This port follows the documented bool form.

**`cr` / `crnl` / `crlf`:** classic `GROUP_APPL` `lineterm` (`xiolayer.c` / `cv_newline`, same SHAs). One ordered field: last active `cr` or `crnl`/`crlf` wins. Write converts NL→CR (`cr`) or NL→CRLF (`crnl`); read converts CR→NL (`cr`) or strips CR (`crnl`). Go-only `crorlf` is a distinct CR-or-LF conversion and is not folded into `cr`/`crnl`. C `TYPE_CONST` rejects values; this port treats `=0` as not selecting, like the shut-* bool form.
