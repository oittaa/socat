...
| UNIX | `unlink-early` (required to replace a leftover path; `reuseaddr` does not unlink), `unlink-close`, `unix-bind-tempname` / `bind-tempname`, `unix-tightsocklen` / `tightsocklen` (classic default tight; `=0` binds and connects with `sizeof(sockaddr_un)`; Windows rejects `=0`), `socktype` / `so-type` |
