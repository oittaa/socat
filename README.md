...
| PTY / TERMIOS | `link`, `cfmakeraw` (distinct from `raw`), `raw`, `rawer`, `sane`, `echo` and other termios flags (`icanon`, `crtscts`, `nl0`/`nl1`, `iuclc`, `pendin`, …), control characters (`vintr`/`intr`, `vmin`, …), baud/`ispeed`/`ospeed` (`b7200` only where `B7200` exists), `tiocswinsz`, `pty-wait-slave`, `ctty`; restore tty on close. Applied in command-line order at classic `PH_FD`. Windows rejects GROUP_TERMIOS options instead of no-op. |
