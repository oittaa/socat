...
| Area | Options |
|------|---------|
| Listen / connect | `reuseaddr` (TCP listen default on; UDP only with `fork` or this option), `so-reuseport`, `fork`, `max-children`, `bind`, `connect-timeout`, `accept-timeout`, `listen-timeout`, `pf`, `ai-addrconfig`, `ipv6-v6only`, `backlog`, `so-linger`/`linger`, `setsockopt-listen` / `sockopt-listen`, `setsockopt` |
