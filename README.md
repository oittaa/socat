...
| IP multicast | `ip-multicast-if`/`multicast-if`, `ip-multicast-loop`/`mcloop`, `ip-multicast-ttl`/`multicast-ttl` (`PH_PASTSOCKET`, `SOL_IP`), `ipv6-multicast-loop`/`mcloop6` (`GROUP_IP6`), IPv4 `ip-add-source-membership`/`source-membership`, IPv6 `ipv6-join-source-group`/`join-source-group`. Linux `ip-freebind` (`PH_PASTSOCKET`) and `ip-transparent` (`PH_PREBIND`). Windows rejects these rather than no-op. `ip-recverr`/`ipv6-recverr` are recognized and rejected (no `MSG_ERRQUEUE` ReadMsg path) |
