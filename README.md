...
- **EXEC `nofork` `fdin`/`fdout`** — classic Dup2's the peer onto `fdi`/`fdo` in the socat process then `exec`s (`xio-progcall.c` `!withfork` at tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same). A failed `exec` can leave that process half-remapped. This port applies the mapping only in the child (ExtraFiles plus the same `dup2` helper as forked high descriptors) so a failed start leaves socat's descriptors unchanged. Unrelated 0/1/2 stay inherited.
