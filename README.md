...
| Area | Options |
|------|---------|
| Listen / connect | `reuseaddr` (TCP listen default on; UDP-LISTEN with `fork` or this option), `so-reuseport`, `fork`, `max-children`, `bind`, `connect-timeout`, `handshake-timeout` (Go extra; TLS/WS/QUIC/PROXY/SOCKS), `accept-timeout`, `listen-timeout`, `pf`, `ai-addrconfig`, `ipv6-v6only`, `backlog`, `so-linger`/`linger`, `sndbuf`/`so-sndbuf`, `rcvbuf`/`so-rcvbuf`, `sndbuf-late`/`so-sndbuf-late`, `rcvbuf-late`/`so-rcvbuf-late`, `bindtodevice`/`so-bindtodevice`/`if`/`interface` (Linux), `setsockopt-listen` / `sockopt-listen`, `setsockopt` |
...
- **`handshake-timeout`** is a Go extra (not in classic `optionnames[]` / `-hhh`). Classic `connect-timeout` (`OPTION_CONNECT_TIMEOUT`, `TYPE_TIMEVAL`, `GROUP_SOCKET`, `PH_PASTSOCKET` at `tag-1.8.1.3` `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is unchanged) aborts the TCP/UDP **connection attempt** only. This port keeps that split: `connect-timeout` bounds dial/accept establishment; `handshake-timeout` bounds TLS, WebSocket, QUIC handshake idle (`HandshakeIdleTimeout`), PROXY, and SOCKS negotiation. On QUIC there is no separate remote connect syscall (`Transport.Dial` does path establishment and the cryptographic handshake together), so `connect-timeout` caps the whole remote attempt (`Dial` + stream open) as well as local UDP bind; it is not used as `HandshakeIdleTimeout`. When both apply to a QUIC client Dial, the earlier positive deadline wins (fresh per retry). The option is rejected on addresses that do not handshake (TCP, UDP, OPEN, EXEC, …). Omitted `handshake-timeout` uses a 30s default so a stalled handshake cannot hang forever; `handshake-timeout=0` disables that handshake bound. quic-go treats `HandshakeIdleTimeout=0` as a 5s default, so this port maps the disabled case to a long explicit duration (1 year) instead of 0. An explicit `connect-timeout` may still cap QUIC connection establishment.
