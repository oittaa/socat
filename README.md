...
- **`retrieve-vlan` on TUN** — classic `xio-tun.c` still calls `_interface_retrieve_vlan` on the TUN char-device fd. `setsockopt(SOL_PACKET, PACKET_AUXDATA)` Error()s and the helper returns 0, so open continues without restoring 802.1Q tags. This port rejects `TUN,retrieve-vlan` up front (`requires an AF_PACKET INTERFACE socket`) instead of that no-op. INTERFACE still enables `PACKET_AUXDATA` and restores a non-zero `tp_vlan_tci` like classic `xioread.c` (tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same).
