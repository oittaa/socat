...
| **Lock file unlink identity** | Identity-safe | Classic `xiounlock` (tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a`) blindly `unlink(2)`s the stored name. This port snapshots the created object after acquire and unlinks `lockfile=` / `waitlock=` / `-L` / `-W` only when `lstat` + `os.SameFile` still match, on normal cleanup, failed-open cleanup, and signal cleanup, so a replacement at the old name is not deleted. |
