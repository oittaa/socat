...
- **EXEC `end-close` + PASTSOCKET** — classic still uses socketpair (`xio-progcall.c`; `end-close` is not `usepipes`) and applies `popts` on `sv[1]` (tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same). This port keeps pipes for LISTEN,fork shared-FD (`EXECENDCLOSE`) and leftover-rejects GROUP_SOCKET options (`option "…" not inquired`) rather than cancelling that safeguard. Unidirectional EXEC and `fdin`/`fdout` still switch to classic socketpair, with the child socket only on the used `-u` direction.
