...
| Area | Options |
|------|---------|
| Listen / connect | `reuseaddr` (TCP listen default on; UDP-LISTEN with `fork` or this option), `so-reuseport`, `fork`, `max-children`, `bind`, `connect-timeout`, `handshake-timeout` (Go extra; TLS/WS/QUIC/PROXY/SOCKS), `accept-timeout`, `listen-timeout`, `pf`, `ai-addrconfig`/`addrconfig` (default on without a family hint; `=0` disables), `ai-passive`/`passive` (listen/bind default on; `=0` binds loopback; explicit on dual-stack connect prefers IPv6), `ai-v4mapped`/`v4mapped` (off unless set; follows classic C, not the man-page IPv6 default), `ai-all` (with v4mapped, also return mapped A records), `res-nsaddr`/`dns`/`nameserver`/`nsaddr`, `res-usevc`/`usevc` (DNS over TCP; `=0` clears even if resolv.conf has `use-vc`), `ipv6-v6only`, `backlog`, `so-linger`/`linger`, `sndbuf`/`so-sndbuf`, `rcvbuf`/`so-rcvbuf`, `sndbuf-late`/`so-sndbuf-late`, `rcvbuf-late`/`so-rcvbuf-late`, `bindtodevice`/`so-bindtodevice`/`if`/`interface` (Linux), `so-debug`/`debug`, `so-dontroute`/`dontroute`, `so-oobinline`/`oobinline`, `so-rcvlowat`/`rcvlowat` (Unix), `so-sndlowat`/`sndlowat` (Darwin/BSD; Linux rejects), `so-priority`/`priority`, `so-passcred`/`passcred`, `so-no-check`/`no-check`/`nocheck` (Linux), `so-detach-filter`/`detach-filter`/`detachfilter` (Linux `SO_DETACH_FILTER`; Darwin/Windows hide and reject), `fiosetown`, `siocspgrp` (Unix FIOSETOWN/SIOCSPGRP SIGIO owner; Windows hides and rejects), `tcp-cork`/`cork`, `tcp-defer-accept`/`defer-accept`, `tcp-linger2`/`linger2`, `tcp-maxseg`/`maxseg`/`mss` (PH_PASTSOCKET), `tcp-maxseg-late`/`maxseg-late`/`mss-late` (PH_CONNECTED), `tcp-quickack`/`quickack`, `tcp-syncnt`/`syncnt`, `tcp-window-clamp`/`window-clamp` (Linux TCP_* except `tcp-maxseg`, which is also honored on other Unix), `sctp-nodelay` / `sctp-maxseg` (Linux SCTP, `PH_PASTSOCKET`; not `TCP_NODELAY`), `setsockopt` / `sockopt` (PH_CONNECTED, dalan value), `setsockopt-int` / `sockopt-int`, `setsockopt-bin` / `sockopt-bin`, `setsockopt-string` / `sockopt-string`, `setsockopt-connected` / `sockopt-conn`, `setsockopt-socket` / `sockopt-sock` (PH_PASTSOCKET), `setsockopt-listen` / `sockopt-listen` (PH_PREBIND) |
...
| TUN / INTERFACE | `tun-name`, `tun-type`, `tun-device`, `iff-up`, `iff-no-pi`, `if-mtu` / `interface-mtu`, other `iff-*` flags, `retrieve-vlan` (INTERFACE only; TYPE_CONST) |
...
| UNIX | `unlink-early` (required to replace a leftover path; `reuseaddr` does not unlink), `unlink-close`, `unix-bind-tempname` / `bind-tempname`, `unix-tightsocklen` / `tightsocklen` (classic `xiosetunix` socklen for both modes, including BSD/Darwin `sun_len`; default tight except FreeBSD/OpenBSD; connect uses classic nonblocking `connect(2)`+poll for `connect-timeout`; Windows hides and rejects the option), `socktype` / `so-type` |
...
| PTY / TERMIOS | `link`, `cfmakeraw` (distinct from `raw`), `raw`, `rawer`, `sane`, complete Linux flags and fields (`icanon`, `crtscts`, `nl0`/`nl1`, `nldly`, `crdly`, `csize`, `iuclc`, `pendin`, …), control characters (`vintr`/`intr`, `vswtc`/`swtch`, …), `termios-setflags`, baud/`ispeed`/`ospeed` (including Linux `b7200` through `BOTHER`), `tiocswinsz`, `pty-wait-slave`, `sitout-eio` (wait on PTY master EIO after the slave closes; omitted or `=0` is EOF), `ctty`; restore tty on close. Applied in command-line order at classic `PH_FD`. Windows rejects GROUP_TERMIOS options instead of no-op. |
| Transfer | `cr`, `crnl`, `crlf`, `crorlf` (Go extra), `ignoreeof`, `readbytes`, `retry`/`forever`/`interval` |
| IP send | `ip-ttl`/`ttl`/`ipttl`, `ip-tos`/`tos`/`iptos`, `ip-options`/`ipoptions`, `ipv6-unicast-hops`, `ipv6-tclass` on TCP, UDP, raw IP, SCTP, TLS/WS/PROXY, and QUIC’s UDP PacketConn. `ip-ttl`/`ip-tos` use `SOL_IP`; family mismatches and Windows-unimplemented send opts error. `ip-hdrincl`/`hdrincl`/`iphdrincl` (`IP_HDRINCL`, `PH_PASTSOCKET` `TYPE_INT`, omitted stores 1) on Unix raw IPv4 only; rejected on TCP/UDP/QUIC/IPv6/Windows. Linux `ip-router-alert`/`routeralert` (`IP_ROUTER_ALERT` on raw IPv4, not `IPPROTO_RAW` / protocol 255); other platforms reject rather than no-op |
| IP multicast / MTU | `ip-multicast-if`/`multicast-if`, `ip-multicast-loop`/`mcloop`, `ip-multicast-ttl`/`multicast-ttl` (`PH_PASTSOCKET`, `SOL_IP`), `ipv6-multicast-loop`/`mcloop6` (`GROUP_IP6`), IPv4 `ip-add-source-membership`/`source-membership`, IPv6 `ipv6-join-source-group`/`join-source-group`. Linux `ip-freebind` (`PH_PASTSOCKET`), `ip-transparent` (`PH_PREBIND`), and `ip-mtu-discover`/`mtudiscover` plus `ipv6-mtu-discover`/`mtudiscover6` (`0..2`, `PH_PASTSOCKET`). Other platforms reject unsupported Linux-only options rather than no-op. `ip-recverr`/`ipv6-recverr` are recognized and rejected (no `MSG_ERRQUEUE` ReadMsg path). `ip-mtu`/`mtu` and `ip-pktoptions`/`pktopts` are recognized and rejected as get-only (Linux `setsockopt` `ENOPROTOOPT`); they are not advertised as setters |
| IP recv ancillary | `so-timestamp`, `ip-pktinfo`/`ippktinfo`, `ip-recvttl`/`iprecvttl`/`ip-recvtos`/`ip-recvopts`, Linux `ip-retopts`/`retopts`/`ipretopts`, `ipv6-recvpktinfo`/`ipv6-recvhoplimit`/`ipv6-recvtclass` on Unix UDP and raw IP (`ReadMsg`; `ip-retopts` is Linux-only). Darwin `ip-recvdstaddr`/`recvdstaddr`/`iprecvdstaddr` (`IP_RECVDSTADDR` → env `IP_DSTADDR`) and `ip-recvif`/`recvif` (`IP_RECVIF` → env `IP_IF`). Rejected on TCP/QUIC/stream sockets, Windows, Linux, other non-Darwin Unix (those two Darwin-only options), and the wrong IP family instead of being accepted as no-ops |
...
**`sitout-eio`:** classic PTY `OFUNC_OFFSET` into `para.exec.sitout_eio` (`xioread.c` `XIOREAD_PTY`, tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same). Omitted or `=0`: EIO on the PTY master is EOF (the slave closed). A positive timeval waits in 10ms ticks (`100*sec + ceil(usec/10000)`), then returns the EIO without a last Read. Value required (`sitout-eio` without `=` fails). Windows rejects the option.
...
- **`res-nsaddr` / `res-usevc` resolver isolation** — classic `xio-ip.c` `xio_res_init` temporarily replaces process-global `_res.nsaddr_list[0]` and other `_res` fields, then restores them after resolution (`tag-1.8.1.3` `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is unchanged). This port uses a per-address Go resolver and never mutates `net.DefaultResolver` or libc `_res`. `res-nsaddr=<ipaddr>[:<port>]` stays classic `TYPE_IP4SOCK` (IPv4 address or hostname; IPv6 nameserver literals are rejected; omitted and zero ports select DNS port 53). `res-usevc`/`usevc` is DNS over TCP via that same per-address `Resolver.Dial` (composed with `res-nsaddr` when both are set). `res-usevc=0` clears `RES_USEVC` the way classic does: queries stay UDP-first, and a truncated UDP response is retried over TCP. A resolv.conf `use-vc` default makes Go dial TCP only; that Dial still speaks DNS-over-TCP framing to Go while sending the first query over UDP and upgrading to TCP when the UDP response has the TC bit. Remaining libc `res-*` flags are rejected rather than applied globally; see [Unsupported / security-related](#unsupported--security-related).
- **`ai-v4mapped` follows C, not the man page, and dials mapped results as AF_INET** — official `doc/socat.yo` says IPv6 addresses default `ai-v4mapped` to 1. Classic C never ORs `AI_V4MAPPED` (`xiogetaddrinfo` in `xio-ip.c` at tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is unchanged). This port follows C: mapping is off unless `ai-v4mapped` is set. Lookups that apply these flags use a `PreferGo` copy of the per-address resolver so the libc cgo path cannot inject `AI_V4MAPPED|AI_ALL` when the options are omitted or `=0` (`LookupResolver` without `netns`/`res-nsaddr`/`res-usevc` still returns `net.DefaultResolver`). When mapping is set, `ip6` lookups synthesize `::ffff:a.b.c.d` for A-only names (`ai-all` also appends mapped A records when AAAA exists). Go's `net` package unmaps those addresses (`IP.String`, `Dial("tcp6")` / `udp6` / `ip6` reports `no suitable address found`), so connect/bind uses AF_INET for hostname lookup results on TCP, SCTP, UDP, QUIC, PROXY HTTP/3, and raw IP instead of classic `connect(2)` on an IPv6 socket. IPv4 literals on a `*6` address still fail family mismatch. `ai-v4mapped=0` disables the mapping. `socat -hhh` describes these effects; it does not name libc `getaddrinfo` flags.
...
- **`retrieve-vlan` on TUN** — classic `xio-tun.c` still calls `_interface_retrieve_vlan` on the TUN char-device fd. `setsockopt(SOL_PACKET, PACKET_AUXDATA)` Error()s and the helper returns 0, so open continues without restoring 802.1Q tags. This port rejects `TUN,retrieve-vlan` at CLI (`not supported with this address type`) instead of that no-op; `openTUN` still rejects a constructed Spec as defense in depth. INTERFACE still enables `PACKET_AUXDATA` and restores a non-zero `tp_vlan_tci` like classic `xioread.c` (tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same). Frames shorter than a 14-byte Ethernet header are left unchanged.
...
- **Get-only, BPF, and obsolete SOL_SOCKET names** — classic `optionnames[]` and `-hhh` advertise `so-error`/`error`, `so-acceptconn`/`acceptconn` as `TYPE_INT` setters (man COMMENTs so-error as read-only and so-acceptconn as “tries to set”). Linux `setsockopt(SO_ERROR)` and `setsockopt(SO_ACCEPTCONN)` return `ENOPROTOOPT`. `so-peercred`/`peercred` is `TYPE_INT3` (a `ucred` struct) whose parser is `#if LATER` in `xioopts.c`, so classic cannot parse a value; man COMMENTs it read-only. `so-attach-filter` is `TYPE_INT` but the kernel wants `struct sock_fprog`; inventing a filter language would need an explicit compatibility decision (same rule as `udp-ignore-peerport`). `so-security-authentication` / `so-security-encryption-network` / `so-security-encryption-transport` are obsolete IPSec-era options (`ENOPROTOOPT`). This port classifies those public spellings as unsupported (unknown option, omitted from `-h`/`-hh`/`-hhh`, not an implementation backlog) rather than advertising setters or silent no-ops. Baseline: tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same tree.
...
- **`ip-mtu` / `ip-pktoptions`** — classic `xio-ip.c` models both as `GROUP_SOCK_IP` `PH_PASTSOCKET` `TYPE_INT` `OFUNC_SOCKOPT` `SOL_IP` setters (`opt_ip_mtu` / `opt_ip_pktoptions`; tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same tree). Official `doc/socat.yo` documents `ip-mtu-discover`, not `ip-mtu`; `ip-pktoptions` appears only as a COMMENT spelling `ip-pktopts` (not in `optionnames[]`). Linux `IP_MTU` and `IP_PKTOPTIONS` are get-only (`setsockopt` returns `ENOPROTOOPT`). This port recognizes the C/`-hhh` names and aliases (`mtu`, `pktopts`, …) and rejects them as get-only instead of advertising a setter or a silent no-op. The man COMMENT spelling `ip-pktopts` is not in `optionnames[]` and is unknown. `ip-hdrincl` is implemented separately (Unix raw IPv4).
- **`ip-retopts` / `ip-router-alert`** — classic COMMENT in the man page; C/`-hhh` still expose them as `TYPE_INT` `SOL_IP` setters. Linux `IP_RETOPTS` is a recv-cmsg boolean like `IP_RECVOPTS` (UDP/raw IPv4). Darwin `IP_RETOPTS` is an IP-options blob, so this port hides and rejects the name there rather than applying an int. Linux `IP_ROUTER_ALERT` works on raw IPv4 except protocol 255 (`IPPROTO_RAW`, classic `IP4-SENDTO:host:255`); TCP/UDP return `EINVAL` and IPv6 raw with `SOL_IP` returns `ENOPROTOOPT`. This port implements both on Linux with those precise rejects. Classic `xiolog_ancillary_ip` does not switch `IP_RETOPTS` cmsgs (they would log as `type_7`); this port logs them as `IP_OPTIONS` like `IP_RECVOPTS`.
...
| **libc `res-*` flags** | Intentionally unsupported | Classic `xio_res_init` mutates process-global `_res` (`res-debug`, `res-defnames`/`defnames`, `res-dnsrch`/`dnsrch`, `res-igntc`/`igntc`, `res-recurse`/`recurse`, `res-stayopen`/`stayopen`, `res-retrans`/`res-maxretrans`/`retrans`, `res-retry`/`res-maxretry`) at tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba` and master `af5388c898c7bb60997935aee93c223deba60c4a`. Go cannot reproduce libc `res_state` per address without a custom DNS client, and this port never mutates `net.DefaultResolver` or `_res`. These spellings are rejected and omitted from `-hhh`, not an implementation backlog. `res-usevc`/`usevc` is implemented per-address. `res-aaonly`/`res-primary` remain foreign (`WITH_AA_ONLY` / `WITH_RES_PRIMARY`). |
...
make test-netns-docker  # root netns= and IP4-RECVFROM tests in a privileged container
...
unsupported exclusions, not backlog items. Remaining libc `res-*` flags that
would mutate process-global `_res` are the same kind of exclusion.
