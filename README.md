...
- **`handshake-timeout`** is a Go extra (not in classic `optionnames[]` / `-hhh`). Classic `connect-timeout` (`OPTION_CONNECT_TIMEOUT`, `TYPE_TIMEVAL`, `GROUP_SOCKET`, `PH_PASTSOCKET` at `tag-1.8.1.3` `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is unchanged) aborts the TCP/UDP **connection attempt** only. This port keeps that split: `connect-timeout` bounds dial/accept establishment (including QUIC UDP bind/listen setup); `handshake-timeout` bounds TLS, WebSocket, QUIC (`HandshakeIdleTimeout`), PROXY, and SOCKS negotiation. The option is rejected on addresses that do not handshake (TCP, UDP, OPEN, EXEC, …). Omitted `handshake-timeout` uses a 30s default so a stalled handshake cannot hang forever; `handshake-timeout=0` disables that bound.
