...
| EXEC / PROCESS | `pipes`, `pty`, `fdin`, `fdout`, `setsid`, `stderr`, `shut-none`, `shut-down`, `shut-close`, `shut-null`, `children-shutup`/`child-shutup`, `chdir`, `umask` (child inherits, then parent restores) |
...
- **EXEC `end-close` + PASTSOCKET** — classic still uses socketpair (`xio-progcall.c`; `end-close` is not `usepipes`) and applies `popts` on `sv[1]` (tag-1.8.1.3 `12c08bf66d709fba17035ce95d85bd218428d9ba`; official master `af5388c898c7bb60997935aee93c223deba60c4a` is the same). This port keeps pipes for LISTEN,fork shared-FD (`EXECENDCLOSE`) and leftover-rejects GROUP_SOCKET options (`option "…" not inquired`) rather than cancelling that safeguard. Unidirectional EXEC, `fdin`/`fdout`, and `pty,fdin`/`fdout` follow classic transport selection (socketpair or PTY) and inherit unrelated standard descriptors.
- **EXEC `fdin`/`fdout` ≥ 10** — classic `xio-progcall.c` `dup2`s an unsigned-short descriptor. This port maps ExtraFiles onto fdi/fdo through `/bin/sh` `exec` redirections so inherited 0/1/2 stay put. Ubuntu `/bin/sh` is dash, whose redirection grammar only accepts a single-digit prefix (`10<&3` is `Syntax error: Bad fd number`). Temporary descriptors therefore stay in 3–9, and `fdin`/`fdout` ≥ 10 fail with `file descriptor N cannot be applied through /bin/sh redirection` rather than a dash syntax error. A child helper that calls `dup2` would be needed for high descriptors.
