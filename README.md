...
| Area | Options |
|------|---------|
| Listen / connect | `reuseaddr` (TCP listen default on; UDP-LISTEN with `fork` or this option), `so-reuseport`, `fork`, `max-children`, `bind`, `connect-timeout`, `handshake-timeout` (Go extra; TLS/WS/QUIC/PROXY/SOCKS), `accept-timeout`, `listen-timeout`, `pf`, `ai-addrconfig`, `ipv6-v6only`, `backlog`, `so-linger`/`linger`, `sndbuf`/`so-sndbuf`, `rcvbuf`/`so-rcvbuf`, `sndbuf-late`/`so-sndbuf-late`, `rcvbuf-late`/`so-rcvbuf-late`, `bindtodevice`/`so-bindtodevice`/`if`/`interface` (Linux), `setsockopt` / `sockopt` (PH_CONNECTED, dalan value), `setsockopt-int` / `sockopt-int`, `setsockopt-bin` / `sockopt-bin`, `setsockopt-string` / `sockopt-string`, `setsockopt-connected` / `sockopt-conn`, `setsockopt-socket` / `sockopt-sock` (PH_PASTSOCKET), `setsockopt-listen` / `sockopt-listen` (PH_PREBIND) |
...
| Files | `rdonly`, `wronly`, `creat`, `excl`, `append`/`o-append` (open(2) on FILE/OPEN/CREATE/GOPEN; `fcntl` `O_APPEND` on FD/STDIO/EXEC/sockets, including `append=0` to clear), `trunc`, `ftruncate`/`truncate`/`ftruncate32`/`ftruncate64` (regular files only), `lseek`/`lseek64`/`seek`/`lseek32` and `-cur`/`-end`/`-set` variants (regular files; last-wins across aliases), `perm`/`mode`, `perm-late`/`mode-late`, `user`/`uid`/`owner`, `user-late`/`uid-l`/`owner-late`, `group`/`gid`, `group-late`/`gid-l`, `perm-early`, `user-early`/`uid-e`, `group-early`/`gid-e` (pre-open chmod/chown of an existing OPEN/CREATE/GOPEN/PIPE name, and chmod/chown of a UNIX socket after bind), `umask`, `nonblock`, `o-direct`/`direct` (Linux), `o-sync`/`sync`, `o-dsync`/`dsync`, `o-rsync`/`rsync` (Linux), `o-noctty`/`noctty`, `o-nofollow`/`nofollow`, `o-directory`/`directory`, `o-largefile`/`largefile`, `async`/`o-async`, `o-noatime`/`noatime`, `fs-noatime`/`ext2-noatime`/`ext3-noatime` (Linux), `unlink-early`, `unlink`/`delete`/`remove`, `unlink-late`, `unlink-close`, `f-setpipe-sz`/`pipesz` (Linux), `setlk` / `setlkw` (read/write variants), `flock`/`flock-ex` / `flock-nb`/`flock-ex-nb` / `flock-sh` / `flock-sh-nb` |
...
**`lseek` / `seek-cur` / `seek-end`:** classic `GROUP_REG|GROUP_BLK` / `PH_LATE`. Every occurrence is applied in command-line order; last-wins across aliases (`lseek`/`lseek64`/`seek`/`lseek32` and the `-set`/`-cur`/`-end` spellings).
...
**`perm-late` / `user-late` / `group-late`:** classic `GROUP_FD` / `PH_LATE` `fchmod`/`fchown`, after `PH_FD` `perm`/`user`/`group`. Command-line order within the phase. `mode-late` and `owner-late` are Go aliases matching `mode`→`perm` and `owner`→`user`.
