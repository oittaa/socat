...
func openSOCKS4(ctx context.Context, s parse.Spec, mode Mode, g *Global, socks4a bool) (*Opened, error) {
	...
	portNum, err := resolvePortNum("tcp", targetPort)
	if err != nil {
	...
	var ip4 [4]byte
	hostName := stripBrackets(targetHost)
	if socks4a {
		ip4 = [4]byte{0, 0, 0, 1}
	...
	network := connectNetworkForType(g, s, socksHost, "tcp")
	...
	dialOnce := func(dctx context.Context) (net.Conn, error) {
		var conn net.Conn
		e := withRetry(dctx, s, g, "SOCKS4", func() error {
...
func openSOCKS5Connect(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	socksHost, socksPort, targetHost, targetPort, err := socksParams(s)
	if err != nil {
		return nil, err
	}
	...
	dialOnce := func(dctx context.Context) (net.Conn, error) {
		var conn net.Conn
		e := withRetry(dctx, s, g, "SOCKS5", func() error {
			c, e := d.DialContext(dctx, network, addr)
			if e != nil {
				return e
			}
