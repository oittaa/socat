...
// decision (same rule as udp-ignore-peerport). so-detach-filter is
// implemented: Linux ignores its TYPE_INT value and can drop a filter
// installed on an inherited socket.
//
// so-security-*: obsolete IPSec-era SO_SECURITY_* (ENOPROTOOPT).
//
// Implemented in this audit, not this map: fiosetown / siocspgrp
// (OFUNC_IOCTL pointer-to-int) and Linux so-detach-filter.
