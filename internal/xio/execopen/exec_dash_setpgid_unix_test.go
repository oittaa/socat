...
func TestEXECDashWithLowFDOutRewritesTargetArgv0(t *testing.T) {
	...
	got := strings.TrimSpace(captureInheritedStdout(t, func() {
		_ = openEXECSpec(t, "EXEC:"+bin+",dash,fdin=3,fdout=4", xio.ModeRDWR)
	}))
	if got != "x-argv0" {
...
func TestEXECSetpgidDoesNotMutateParentOnNofork(t *testing.T) {
	...
	peer := relay.FDStream{R: os.Stdin, W: os.Stdout, C: xio.NopCloser{}}
	if err := runExecNoFork(context.Background(), peer, prepared.Config, nil, xio.ModeRDWR); err != nil {
		t.Fatal(err)
	}
...
func readExecStdout(t *testing.T, spec string) string {
	...
	o, err := OpenChannel(ctx, ch, xio.ModeRead, nil)
	if err != nil {
		t.Fatal(err)
	}
