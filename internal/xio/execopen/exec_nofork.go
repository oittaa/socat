...
func runExecNoFork(ctx context.Context, peer relay.Stream, config addrconfig.Address, g *xio.Global, mode xio.Mode) error {
...
func commandForConfiguredExec(ctx context.Context, config addrconfig.Address, opts xio.Options) (*exec.Cmd, error) {
...
func peerStdioFiles(st relay.Stream, mode xio.Mode) (in, out *os.File, err error) {
...
func noForkPeerExtraFiles(st relay.Stream, mode xio.Mode) (files []*os.File, sameFD bool, err error) {
	r, w, single, err := streamRWFiles(st)
	if err != nil {
...
func (c *execChild) waitNoFork() error {
	...
	if c.cmd.Process != nil {
		xio.UnregisterChildSignals(c.cmd.Process.Pid)
	}
