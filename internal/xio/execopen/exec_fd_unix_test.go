...
func openEXECSpec(t *testing.T, specText string, mode xio.Mode) *xio.Opened {
	...
	o, err := OpenSpec(context.Background(), spec, mode, xio.NewSession(xio.Options{Linger: time.Second}, logx.New()))
	if err != nil {
		t.Fatal(err)
	}
...
func TestOpenSpecFDInOnlyModeWriteUnix(t *testing.T) {
	...
	got := captureInheritedStdout(t, func() {
		o := openEXECSpec(t, "SYSTEM:cat <&3,fdin=3", xio.ModeWrite)
...
func TestOpenSpecStderrCustomFDOutUnix(t *testing.T) {
	...
	inherited = captureInheritedStdout(t, func() {
		o := openEXECSpec(t, "SYSTEM:printf O; printf D >&4; printf E >&2,fdin=3,fdout=4,stderr", xio.ModeRDWR)
...
func TestOpenSpecStderrHighFDOutUnix(t *testing.T) {
	...
	var inherited, relayed string
	inherited = captureInheritedStdout(t, func() {
		o := openEXECSpec(t, "EXEC:/bin/bash -c \\\"printf O; printf D >&10; printf E >&2\\\",fdin=9,fdout=10,stderr", xio.ModeRDWR)
		relayed = string(readStreamBytes(t, o.Stream(), 3*time.Second))
	})
	if inherited != "O" {
		t.Fatalf("inherited stdout %q want O", inherited)
...
func TestOpenSpecChdirWithFDInFDOutUnix(t *testing.T) {
	dir := t.TempDir()
	got := captureInheritedStdout(t, func() {
		_ = openEXECSpec(t, "SYSTEM:pwd,chdir="+dir+",fdin=3,fdout=4", xio.ModeRDWR)
	})
...
func TestOpenSpecChdirWithHighFDOutUnix(t *testing.T) {
	...
	dir := t.TempDir()
	o := openEXECSpec(t, "EXEC:/bin/bash -c \\\"pwd >&10\\\",chdir="+dir+",fdout=10", xio.ModeRead)
...
func TestOpenSpecEXECSocktypeDgramUnix(t *testing.T) {
	...
	const payload = "hello"
	got := captureInheritedStdout(t, func() {
		o := openEXECSpec(t, "EXEC:/bin/cat,socktype="+strconv.Itoa(syscall.SOCK_DGRAM), xio.ModeWrite)
		if _, err := o.Stream().Write([]byte(payload)); err != nil {
			t.Fatal(err)
		}
		if err := o.Stream().ShutdownWrite(); err != nil {
...
func parentSocketType(t *testing.T, o *xio.Opened) (int, error) {
	t.Helper()
	f := asOSFile(o.Stream())
	if f == nil {
...
func TestOpenSpecEXECEndCloseUsesSocketpairUnix(t *testing.T) {
	...
	o := openEXECSpec(t, "EXEC:true,end-close", xio.ModeRDWR)
...
func TestOpenSpecEXECPipesIsNotSocketUnix(t *testing.T) {
	o := openEXECSpec(t, "EXEC:true,pipes", xio.ModeRDWR)
...
func noForkPipePeer(t *testing.T) (peer relay.Stream, inW, outR, outW *os.File) {
	...
	return relay.FDStream{R: inR, W: outW, C: xio.NopCloser{}}, inW, outR, outW
}
...
func runPreparedNoFork(t *testing.T, peer relay.Stream, s parse.Spec, g *xio.Global, mode xio.Mode) {
	t.Helper()
...
func TestRunExecNoForkTrueWithCustomFDsUnix(t *testing.T) {
	...
	runPreparedNoFork(t, peer, s, g, xio.ModeRDWR)
...
func TestRunExecNoForkDashRewritesTargetArgv0Unix(t *testing.T) {
	...
	got := strings.TrimSpace(captureInheritedStdout(t, func() {
		...
		runPreparedNoFork(t, peer, s, nil, xio.ModeRDWR)
	}))
	if got != "x-argv0" {
