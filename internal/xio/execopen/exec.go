...
func openEXEC(ctx context.Context, s addrconfig.Address, mode xio.Mode, g *xio.Global) (*xio.Opened, error) {
...
func openSYSTEM(ctx context.Context, s addrconfig.Address, mode xio.Mode, g *xio.Global) (*xio.Opened, error) {
...
func openSHELL(ctx context.Context, s addrconfig.Address, mode xio.Mode, g *xio.Global) (*xio.Opened, error) {
	return startCmd(ctx, s, mode, g, configuredShellCommand(ctx, s.Process, g.Options()))
}

func configuredShellCommand(ctx context.Context, config addrconfig.Process, opts xio.Options) *exec.Cmd {
...
func startProcess(ctx context.Context, s addrconfig.Address, mode xio.Mode, g *xio.Global, useShell bool) (*xio.Opened, error) {
	var cmd *exec.Cmd
	if useShell {
...
func applyExecProcessAttrs(config addrconfig.Address, cmd *exec.Cmd, g *xio.Global, fdRedirect bool) error {
...
func (c *execChild) startForked(ctx context.Context) (*xio.Opened, error) {
	if c.usePty {
	...
	return c.finish(stream, cleanup, c.mode == xio.ModeWrite, nil)
}
...
func startCmd(ctx context.Context, s addrconfig.Address, mode xio.Mode, g *xio.Global, cmd *exec.Cmd) (*xio.Opened, error) {
	...
	if c.config.Common.NoFork.Value {
		...
		config := c.config
		return xio.NewDeferredNoFork("EXEC-nofork", config), nil
	}
...
func validateProcessFDOptions(mode xio.Mode, fdin, fdout string) error {
	if mode == xio.ModeWrite && fdout != "" {
		return fmt.Errorf("fdout is not valid in a write-only process address")
	}
	if mode == xio.ModeRead && fdin != "" {
		return fmt.Errorf("fdin is not valid in a read-only process address")
	}
	return nil
}

func processFDPairConfig(config addrconfig.Process, mode xio.Mode) (fdin, fdout string, err error) {
	if config.FDIn.Set {
...
func startCmdPipes(config addrconfig.Address, mode xio.Mode, cmd *exec.Cmd, fdRedirect bool) (relay.Stream, []func(), []*os.File, error) {
	...
	if needIn {
		...
		if err := xio.ApplyConfiguredFDOptions(childStdin, config.File, xio.FDSkip{}); err != nil {
	...
	if needOut {
		...
		if err := xio.ApplyConfiguredFDOptions(childStdout, config.File, xio.FDSkip{}); err != nil {
			closeFiles(parentFiles)
			closeFiles(childFiles)
			return nil, nil, nil, err
		}
...
func startCmdSocketpair(config addrconfig.Address, mode xio.Mode, cmd *exec.Cmd, fdRedirect bool) (relay.Stream, []func(), *os.File, error) {
	stype, _, err := xio.SocketTypeOption(config, syscall.SOCK_STREAM)
	if err != nil {
		return nil, nil, nil, err
	}
	...
	if err := sockopt.ApplySocketOptions(int(child.Fd()), config); err != nil {
		_ = parent.Close()
		_ = child.Close()
		return nil, nil, nil, err
	}
...
func (c *execChild) start(ctx context.Context) error {
	...
	setCloexecAllFrom(3)
	var startErr error
	if err := xio.WithConfiguredUmask(c.config.File, func() error {
	...
	if err := xio.RegisterExecParentSignals(c.config, c.cmd, c.g); err != nil {
		c.killWait()
		return err
	}
	return nil
...
func (w *execWaitState) recordExit(g *xio.Global) {
	w.mu.Lock()
	code := w.exitCode
	werr := w.waitErr
	w.mu.Unlock()
...
func (c *execChild) finish(stream relay.Stream, cleanup []func(), waitChild bool, done chan struct{}) (*xio.Opened, error) {
	return c.finishStream(stream, cleanup, waitChild, done, xio.SetupStream)
}

func (c *execChild) finishAfterFD(stream relay.Stream, cleanup []func(), waitChild bool, done chan struct{}) (*xio.Opened, error) {
	return c.finishStream(stream, cleanup, waitChild, done, xio.WrapAfterFD)
}

func (c *execChild) finishStream(stream relay.Stream, cleanup []func(), waitChild bool, done chan struct{}, wrap func(addrconfig.Address, relay.Stream) (relay.Stream, error)) (*xio.Opened, error) {
...
func pipeDirections(mode xio.Mode) (needIn, needOut bool) {
	switch mode {
	case xio.ModeRead:
		return false, true
	case xio.ModeWrite:
