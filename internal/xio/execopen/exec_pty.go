...
func applyExecPtySession(cmd *exec.Cmd, config addrconfig.Process, g *xio.Global) {
...
func openExecPTYPair(cmd *exec.Cmd, config addrconfig.Address, g *xio.Global) (*os.File, *os.File, func(), error) {
	...
	if err := termios.ApplyConfiguredTermios(int(slave.Fd()), config.Terminal); err != nil {
	...
	if err := termios.ApplyConfiguredTermios(int(master.Fd()), config.Terminal); err != nil {
		logx.CloseQuiet(master)
		logx.CloseQuiet(slave)
		return nil, nil, nil, err
	}
	...
	if err := xio.ApplyConfiguredNamedAttrs(slave.Name(), slave, config.File); err != nil {
		logx.CloseQuiet(master)
		logx.CloseQuiet(slave)
		return nil, nil, nil, err
	}
...
func (c *execChild) startPtyFDRedirect(ctx context.Context) (*xio.Opened, error) {
	...
	switch c.mode {
	case xio.ModeWrite:
		w := xio.NewHalfCloseWriter(master)
...
func (c *execChild) startPty(ctx context.Context) (*xio.Opened, error) {
	if c.fdRedirect {
...
func (c *execChild) startOnPTY(ctx context.Context) (*os.File, *os.File, func(), error) {
	...
	if err := termios.ApplyConfiguredTermios(int(slave.Fd()), c.config.Terminal); err != nil {
		logx.CloseQuiet(master)
		logx.CloseQuiet(slave)
		return nil, nil, nil, err
	}
	// Before Start: Darwin TIOCSETA on the controller flushes t_outq.
	if err := termios.ApplyConfiguredTermios(int(master.Fd()), c.config.Terminal); err != nil {
		logx.CloseQuiet(master)
		logx.CloseQuiet(slave)
		return nil, nil, nil, err
	}
	if err := xio.ApplyConfiguredNamedAttrs(slave.Name(), slave, c.config.File); err != nil {
		logx.CloseQuiet(master)
		logx.CloseQuiet(slave)
		return nil, nil, nil, err
	}
...
func applyPtyMasterLifecycle(config addrconfig.Address, ptmx *os.File) error {
	return xio.ApplyConfiguredFDOptions(ptmx, config.File, xio.FDSkipOwner)
}
