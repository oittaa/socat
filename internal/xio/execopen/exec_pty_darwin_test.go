...
func TestDarwinEXECPtyDrainsOutputAfterChildExit(t *testing.T) {
	...
	for _, opt := range []string{"pty", "ptmx", "openpty"} {
		t.Run(opt, func(t *testing.T) {
			for i := 0; i < 10; i++ {
				o := openEXECSpec(t, "EXEC:"+bin+","+opt+",rawer,echo=0", xio.ModeRDWR)
...
func TestDarwinEXECPtySilentChildReachesEOF(t *testing.T) {
	o := openEXECSpec(t, "SYSTEM:true,pty,rawer,echo=0", xio.ModeRDWR)
	waitExecPTYChild(t, o)
...
func waitExecPTYChild(t *testing.T, o *xio.Opened) {
	t.Helper()
	select {
	case <-o.ChildDone():
