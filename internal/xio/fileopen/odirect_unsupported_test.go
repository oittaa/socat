...
func TestPIPEODirectRejectedWhenUnsupported(t *testing.T) {
	...
	if _, err := xio.OpenSpec(context.Background(), named, xio.ModeRead, nil); err == nil || !strings.Contains(err.Error(), "not supported") {
		t.Fatalf("named PIPE o-direct: %v", err)
	}
	unnamed := parse.Spec{Type: "PIPE", Options: []parse.Option{{Name: "o-direct"}}}
	if _, err := xio.OpenSpec(context.Background(), unnamed, xio.ModeRDWR, nil); err == nil || !strings.Contains(err.Error(), "not supported") {
