...
func TestQUICTargetConnect(t *testing.T) {
	...
	host, port, err := quicTarget(mustAddr(t, s), false)
	if err != nil {
		t.Fatal(err)
	}
...
func TestQUICTargetListen(t *testing.T) {
	...
	_, port, err := quicTarget(mustAddr(t, s), true)
	if err != nil {
		t.Fatal(err)
	}
	if port != "4433" {
...
func TestQUICConfigRequiresTLS13Maximum(t *testing.T) {
	...
	for _, version := range []uint16{tls.VersionTLS10, tls.VersionTLS11, tls.VersionTLS12} {
		_, err := quicConfig(t.Context(), mustAddr(t, s), &tls.Config{MaxVersion: version})
...
func TestQUICConfigHandshakeIdleTimeoutFromHandshakeTimeout(t *testing.T) {
	...
	setup, err := quicConfig(t.Context(), mustAddr(t, s), &tls.Config{})
	if err != nil {
		t.Fatal(err)
	}
