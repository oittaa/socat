...
func handleTimestamp(data []byte, g Session) {
...
func handleIPv4Cmsg(typ int32, data []byte, g Session) {
	...
	if typ == unix.IP_TTL || typ == unix.IP_RECVTTL {
		...
		sessionSet(g, "IP_TTL", val)
		if g != nil {
			g.Noticef("Ancillary message: ttl=%s", val)
		}
		return
	}
	...
	if typ == unix.IP_TOS || typ == unix.IP_RECVTOS {
		...
		sessionSet(g, "IP_TOS", val)
		if g != nil {
			g.Noticef("Ancillary message: tos=%s", val)
		}
		return
	}
	...
	case unix.IP_PKTINFO:
		...
		sessionSet(g, "IP_IF", ifname)
		sessionSet(g, "IP_LOCADDR", loc)
		sessionSet(g, "IP_DSTADDR", dst)
		if g != nil {
			g.Noticef("Ancillary message: interface %q, locaddr=%s, dstaddr=%s", ifname, loc, dst)
		}
	}
}

func handleIPv4OptionsCmsg(data []byte, g Session) {
	val := hexCmsg(data)
	LogAncillary(g, "IP_OPTIONS", "options", val)
	sessionSet(g, "IP_OPTIONS", val)
}

func handleIPv6Cmsg(typ int32, data []byte, g Session) {
	switch typ {
	case unix.IPV6_PKTINFO:
		...
		sessionSet(g, "IPV6_DSTADDR", br)
		sessionSet(g, "IPV6_IF", ifname)
	case unix.IPV6_HOPLIMIT:
		val := strconv.Itoa(cmsgInt(data))
		LogAncillary(g, "IPV6_HOPLIMIT", "hoplimit", val)
		sessionSet(g, "IPV6_HOPLIMIT", val)
...
func handleIPv6ExtHdrCmsg(typeName, shortName string, data []byte, g Session) {
	LogAncillary(g, typeName, shortName, hexCmsg(data))
}

func handleIPv6GenericCmsg(typ int32, data []byte, g Session) {
...
func LogAncillary(g Session, typ, name, val string) {
	if g != nil {
		g.Infof("ancillary message: %s: %s=%s", typ, name, val)
	}
}
