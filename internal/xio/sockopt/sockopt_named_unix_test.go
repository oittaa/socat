...
func TestApplyTCPConnOptsNamedConnectedOnUDPUnix(t *testing.T) {
	...
	err = xio.ApplyTCPConnOpts(mustDecodeAddress(t, spec), c)
	if err == nil {
...
func TestFDRejectsMaxsegLateUnix(t *testing.T) {
	...
	err = sockopt.RejectGenericSetsockoptPhases(mustDecodeAddress(t, spec), "FD", sockopt.SockoptPhasePrebind, sockopt.SockoptPhaseConnected)
