...
func TestSocketUnixRangeRejected(t *testing.T) {
	...
	_, err := openSpec(context.Background(), mustSocketSpec(t, socketDgramSpec("SOCKET-DATAGRAM", unix.AF_UNIX, unix.SOCK_DGRAM, 0,
...
func TestSocketRecvWriteModeRejected(t *testing.T) {
	_, err := openSpec(context.Background(), mustSocketSpec(t, socketDgramSpec("SOCKET-RECV", unix.AF_INET, unix.SOCK_DGRAM, unix.IPPROTO_UDP,
