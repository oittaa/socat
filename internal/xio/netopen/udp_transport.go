...
func listenPacketForSpec(ctx context.Context, network, address string, s addrconfig.Address) (net.PacketConn, error) {
...
func dialUDPForSpec(req dialRequest, laddr net.Addr, remote string) (net.Conn, error) {
	if host, port, err := net.SplitHostPort(remote); err == nil {
		...
		netw, ip, resolveErr := xio.LookupDialIP(req.ctx, req.config, req.network, stripped)
		if resolveErr != nil {
	...
	d := net.Dialer{
		Timeout:   req.timeout,
		LocalAddr: matched,
		Control:   xio.DialControl(req.config, req.network, req.control),
