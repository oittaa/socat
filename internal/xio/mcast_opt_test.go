...
func membershipJoins(config addrconfig.Address) []membershipJoin {
...
func TestMembershipJoinsCollectsAllInOptionOrder(t *testing.T) {
	...
	if got[0].family != membershipFamilyIPv4 || got[0].name != "ip-add-membership" || got[0].spec != "224.0.0.1:lo" {
		t.Fatalf("ipv4 join=%+v", got[0])
	}
	if got[1].family != membershipFamilyIPv6 || got[1].name != "ipv6-join-group" || got[1].spec != "ff02::2:eth0" {
...
func TestMembershipJoinsRecognizesClassicAliases(t *testing.T) {
	...
	for i := range wantFam {
		if got[i].family != wantFam[i] || got[i].spec != wantSpec[i] || got[i].name != wantName[i] {
			t.Fatalf("alias[%d]=%+v want family=%v spec=%q name=%q", i, got[i], wantFam[i], wantSpec[i], wantName[i])
		}
	}
}
