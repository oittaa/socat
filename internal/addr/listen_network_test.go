...
func TestListenNetworkPrecedence(t *testing.T) {
	...
	// Global -6 overrides env=4
	t.Setenv("SOCAT_DEFAULT_LISTEN_IP", "4")
	if got := listenNetwork(&Global{IPVersion: IPv6}, parse.Spec{}); got != "tcp6" {
	...
	if got := listenNetwork(&Global{IPVersion: IPv6}, s); got != "tcp4" {
		t.Fatalf("pf=ip4: got %q want tcp4", got)
	}
	s = parse.Spec{Options: []parse.Option{{Name: "pf", Value: "ip6", Has: true}}}
	if got := listenNetwork(&Global{IPVersion: IPv4}, s); got != "tcp6" {
		t.Fatalf("pf=ip6: got %q want tcp6", got)
	}

	// -0 → dual-stack "tcp"
	t.Setenv("SOCAT_DEFAULT_LISTEN_IP", "4")
	if got := listenNetwork(&Global{IPVersion: IPvAny}, parse.Spec{}); got != "tcp" {
...
func TestOpenSSLListenHonorsDefaultListenIP6(t *testing.T) {
	...
	o, err := openOpenSSLListen(ctx, s, ModeRead, &Global{})
	if err != nil {
