...
func openSocketConnect(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	...
	if bind := s.OptionValue("bind", ""); bind != "" {
		bdata, berr := parseSocatData(bind)
		if berr != nil {
			unix.Close(fd)
			return nil, berr
		}
	...
	_ = mode
	_ = g
	return &Opened{Stream: st, Label: "SOCKET-CONNECT"}, nil
}

// SOCKET-LISTEN:<domain>:<protocol>:<local-address>
func openSocketListen(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	domain, proto, addrData, err := parseSocketParams(s, 3)
	if err != nil {
		return nil, err
	}
	...
	_ = mode
	_ = g
	return &Opened{Stream: st, Label: "SOCKET-LISTEN"}, nil
}

// SOCKET-SENDTO / SOCKET-DATAGRAM: domain:type:protocol:remote
func openSocketSendto(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	return openSocketDgram(ctx, s, mode, g, false)
}

func openSocketDatagram(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	return openSocketDgram(ctx, s, mode, g, true)
}

func openSocketDgram(ctx context.Context, s parse.Spec, mode Mode, g *Global, connected bool) (*Opened, error) {
	if len(s.Params) < 4 {
	...
	addrData, err := parseSocatData(addrText)
	if err != nil {
		return nil, err
	}
	sa, salen, err := buildSockaddr(domain, addrData)
...
func openSocketRecv(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	return openSocketRecvCommon(ctx, s, mode, g, false)
}
func openSocketRecvfrom(ctx context.Context, s parse.Spec, mode Mode, g *Global) (*Opened, error) {
	return openSocketRecvCommon(ctx, s, mode, g, true)
}

func openSocketRecvCommon(ctx context.Context, s parse.Spec, mode Mode, g *Global, from bool) (*Opened, error) {
	if len(s.Params) < 4 {
		return nil, fmt.Errorf("%s requires domain:type:protocol:address", s.Type)
	}
	...
	_ = mode
	_ = g
	return &Opened{Stream: wrapped, Label: s.Type}, nil
}
...
func parseSocketParams(s parse.Spec, n int) (domain, proto int, addr []byte, err error) {
	...
	addr, err = parseSocatData(addrText)
	if err != nil {
		return 0, 0, nil, err
	}
	if domain == 0 {
