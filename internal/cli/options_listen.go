...
func listenOptionGroups() []helpOptGroup {
	return []helpOptGroup{
		{"Listen and connect", []helpOpt{
			...
			{name: "ai-addrconfig", desc: "skip addresses whose family is not configured on this host (default on without a family hint; =0 disables)", aliases: []string{"addrconfig"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-passive", desc: "use wildcard addresses for listen/bind (default); =0 uses loopback, and dual-stack connect prefers IPv6", aliases: []string{"passive"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-v4mapped", desc: "map IPv4 answers onto IPv6 lookups (off unless set)", aliases: []string{"v4mapped"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-all", desc: "with ai-v4mapped, also return mapped IPv4 answers when IPv6 answers exist", addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
...
func transferOptionGroups() []helpOptGroup {
	return []helpOptGroup{
		{"Transfer", []helpOpt{
			...
			{name: "lockfile", desc: "create lock file or fail if it exists (like -L)", validate: validateRequiredString},
			{name: "waitlock", desc: "wait until lock file is gone, then create it (1s poll; CLI -W uses 100ms)", validate: validateRequiredString},
		}},
	}
}
