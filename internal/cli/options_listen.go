...
func listenOptionGroups() []helpOptGroup {
	return []helpOptGroup{
		{"Listen and connect", []helpOpt{
			...
			{name: "ai-addrconfig", desc: "getaddrinfo AI_ADDRCONFIG (default on without family hint; =0 disables)", aliases: []string{"addrconfig"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-passive", desc: "getaddrinfo AI_PASSIVE (listen/bind default on; =0 binds loopback; explicit on dual-stack connect prefers IPv6)", aliases: []string{"passive"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-v4mapped", desc: "getaddrinfo AI_V4MAPPED (off unless set; classic C does not default it on)", aliases: []string{"v4mapped"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-all", desc: "getaddrinfo AI_ALL (with v4mapped, return mapped A records too)", addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
