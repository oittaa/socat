...
func listenOptionGroups() []helpOptGroup {
	return []helpOptGroup{
		{"Listen and connect", []helpOpt{
			...
			{name: "ai-addrconfig", desc: "getaddrinfo AI_ADDRCONFIG", aliases: []string{"addrconfig"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-passive", desc: "getaddrinfo AI_PASSIVE (listen/bind default on; =0 binds loopback)", aliases: []string{"passive"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
			{name: "ai-v4mapped", desc: "getaddrinfo AI_V4MAPPED (IPv6 default on)", aliases: []string{"v4mapped"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
