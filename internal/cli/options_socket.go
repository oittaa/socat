...
func socketOptionGroups() []helpOptGroup {
	return []helpOptGroup{
		{"Sockets", []helpOpt{
			...
			{name: "so-detach-filter", desc: "SO_DETACH_FILTER (Linux)", aliases: []string{"detach-filter", "detachfilter"}, validate: validateOptionalSignedInteger},
			...
			{name: "tcp-maxseg", desc: "TCP_MAXSEG after socket", aliases: []string{"maxseg", "mss"}, addressTypes: tcpStreamAddressTypes()},
			{name: "tcp-maxseg-late", desc: "TCP_MAXSEG after connect or accept", aliases: []string{"maxseg-late", "mss-late"}, addressTypes: tcpStreamAddressTypes()},
			...
			{name: "nopush", desc: "TCP_NOPUSH (Darwin)", aliases: []string{"tcp-nopush"}, addressTypes: tcpStreamAddressTypes()},
			{name: "noopt", desc: "TCP_NOOPT (Darwin)", aliases: []string{"tcp-noopt"}, addressTypes: tcpStreamAddressTypes()},
			{name: "sctp-nodelay", desc: "SCTP_NODELAY (Linux)", implementationGroups: []string{xio.GroupSCTP}},
			{name: "sctp-maxseg", desc: "SCTP_MAXSEG after socket", implementationGroups: []string{xio.GroupSCTP}},
			...
			{name: "ip-router-alert", desc: "IP_ROUTER_ALERT (Linux raw IPv4; not IPPROTO_RAW)", aliases: []string{"iprouteralert", "routeralert"}, implementationGroups: []string{xio.GroupRawIP}, validate: validateOptionalSignedInteger},
			...
			{name: "ip-retopts", desc: "IP_RETOPTS (Linux)", aliases: []string{"retopts", "ipretopts"}, implementationGroups: xio.IPAncillaryImplementationGroups("ip-retopts")},
			...
			{name: "ipv6-recvdstopts", desc: "IPV6_RECVDSTOPTS (Linux)", aliases: []string{"recvdstopts"}, implementationGroups: xio.IPAncillaryImplementationGroups("ipv6-recvdstopts")},
			{name: "ipv6-recvhopopts", desc: "IPV6_RECVHOPOPTS (Linux)", aliases: []string{"recvhopopts"}, implementationGroups: xio.IPAncillaryImplementationGroups("ipv6-recvhopopts")},
			{name: "ipv6-recvrthdr", desc: "IPV6_RECVRTHDR (Linux)", aliases: []string{"recvrthdr"}, implementationGroups: xio.IPAncillaryImplementationGroups("ipv6-recvrthdr")},
			{name: "ipv6-recvpathmtu", desc: "IPV6_RECVPATHMTU (Linux)", implementationGroups: xio.IPAncillaryImplementationGroups("ipv6-recvpathmtu")},
			...
			{name: "res-usevc", desc: "per-address DNS over TCP (RES_USEVC); =0 restores UDP then TCP even if resolv.conf has use-vc", aliases: []string{"usevc"}, addressTypes: resolverAddressTypes(), implementationGroups: resolverImplementationGroups(), validate: validateOptionalBool},
