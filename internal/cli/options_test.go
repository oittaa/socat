...
func TestValidateAddressOptions(t *testing.T) {
	tests := []struct {
		...
		{name: "append-on-tcp", spec: "TCP:localhost:1,append", wantErr: "not supported"},
		...
		{name: "socket-timeout-on-websocket", spec: "WS:localhost:1,rcvtimeo=0.1", wantErr: "not supported"},
		{name: "socket-timeout-on-quic", spec: "QUIC:localhost:1,sndtimeo=0.1", wantErr: "not supported"},
		...
		{name: "readbytes-negative", spec: "OPEN:file,readbytes=-1", wantErr: "invalid readbytes"},
